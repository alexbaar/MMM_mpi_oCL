//credit: http://www.hpcc.unn.ru/mskurs/LAB/ENG/DOC/Lab02.pdf 
...
/// Function for simple initialization of matrix elements; values just like in the sequential
void DummyDataInitialization (double* pAMatrix, double* pBMatrix,int Size){
 ...
 for (i=0; i<Size; i++)
    for (j=0; j<Size; j++) {
        pAMatrix[i*Size+j] = counter++;
        pBMatrix[i*Size+j] = counter2++;
    }
} 
...
void RandomDataInitialization (double* pAMatrix, double* pBMatrix,
    ...
    for (int i = 0; i < Size; i++) {
        for (int j = 0; j < Size; j++) {
		    pAMatrix[i*Size+j] = {static_cast<double>(99)}; // Set to a large negative value (guaranteed to be outside the range)
            pBMatrix[i*Size+j] = {static_cast<double>(99)};
	    }
    }
    ...
    for (i=0; i<Size; i++){
        for (j=0; j<Size; j++) {
            pAMatrix[i*Size+j] = dist(e2);
            pBMatrix[i*Size+j] = dist(e2);
        }
    }
}
...
// time start here, after random nr generation
